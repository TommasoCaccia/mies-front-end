...
const Card = ({number, title, content, imgSrc, imgStyle, link}) => {
    return (
        <figure className={`${classes.card} ${classes.shapeBoxHalf}`}>
            <img src={imgSrc} className={`${classes.logo} d-block`} alt="Logo" style={imgStyle}/>
            <div
                className={`${classes.brkAbsOverlay} ${classes.zIndex0} ${classes.bgBlack} ${classes.opacity60}`}></div>
            <figcaption>
                <div className={classes.showCont}>
                    <h3 className={classes.cardNo}>{number}</h3>
                    <h4 className={`${classes.cardMainTitle}`} dangerouslySetInnerHTML={{__html: title}}></h4>
                </div>
                <p className={classes.cardContent} dangerouslySetInnerHTML={{__html: content}}></p>
                <a href={link} className={classes.readMoreBtn}>Approfondisci</a>
            </figcaption>
    ...
    return (
        <div className={`box-wrapper ${classes.boxWrapper}`}>
            ...
            <Card
                ...
                link="/servizi/altro-servizio"
                imgStyle={{width: '1570px', height: 'auto', marginLeft: '-380px', marginTop: '-20px'}}
            />
            <Card
                number="03"
            ...
            <Card
                ...
                link="/servizi/quarto-servizio"
                imgStyle={{width: '1500px', height: 'auto', marginLeft: '-1100px'}}
            />
            <Card
                number="05"
